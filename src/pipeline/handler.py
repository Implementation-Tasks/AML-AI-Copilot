...
def _run_qubo_stage(
    ...
    In production this graph is populated from Etherscan + The Graph live data.
    In demo/test mode a minimal synthetic graph is used so the pipeline runs
    end-to-end without external API calls.
    ...
    # ── Build minimal graph centred on target wallet ──────────────────────────
    # In a full production deployment, replace this block with:
    #   from src.data.elliptic_loader import EllipticLoader
    #   graph = EllipticLoader().build_graph_for_wallet(wallet_address)
    G = nx.scale_free_graph(50, seed=int(uuid.uuid4().int % 2**31)).to_directed()
    nodes = list(G.nodes())
    target_node = nodes[0] if nodes else 0

    # Assign wallet labels — target wallet is treated as known illicit candidate
    node_to_addr = {n: (wallet_address if n == target_node else f"0xSYNTH{n:04x}") for n in nodes}
    nx.set_node_attributes(G, {n: {"address": addr} for n, addr in node_to_addr.items()})

    tx_graph = TransactionGraph(
        graph=G,
        known_illicit=[wallet_address],
        known_licit=[f"0xSYNTH{n:04x}" for n in nodes if n != target_node],
        source_dataset="quapp_live",
    )
    ...
    risk_score = opt_result.risk_scores.get(wallet_address, 0.0)
    # If wallet was flagged at all, treat its score as high confidence
    if wallet_address in opt_result.flagged_wallets:
