...
from src.security import sign_payload, threshold_routing
...
class EtherscanAPITool(BaseTool):
    ...
    def _execute_evm(self, wallet_address: str, chain: str, max_hops: int) -> ToolOutput:
        """Fetch EVM chain (ETH/BSC) transactions with anti-spoofing validation.

        Bottleneck 3 (slide 16): Multi-node RPC consensus + payload signature
        verification before data enters the graph builder.
        """
        api_key  = ETHERSCAN_API_KEY
        base_url = self.CHAIN_APIS.get(chain, self.CHAIN_APIS["eth"])
        coin     = self.CHAIN_COINS.get(chain, "eth")

        # ── Anti-Spoofing: Multi-Node RPC Consensus (slide 16) ────────────────────
        # In production: query Etherscan + Alchemy + The Graph and compare
        # state hashes before accepting any data.
        # Here we simulate consensus with a lightweight state-hash check
        # on the primary API response.
        raw_response_node_a = self._fetch_evm_txs(wallet_address, base_url, api_key, page=1, limit=5)
        # Simulate second node (same endpoint different page as proxy; in
        # production this would be a different RPC provider).
        raw_response_node_b = self._fetch_evm_txs(wallet_address, base_url, api_key, page=1, limit=5)

        # Compute state hash from each node response for consensus check
        def _state_hash(resp: list) -> str:
            canonical = json.dumps(
                sorted([tx.get("hash", "") for tx in resp[:10]]),
                sort_keys=True,
            )
            return hashlib.sha256(canonical.encode()).hexdigest()[:16]

        hash_a = _state_hash(raw_response_node_a)
        hash_b = _state_hash(raw_response_node_b)
        rpc_responses = [
            {"state_hash": hash_a, "source": "primary_rpc"},
            {"state_hash": hash_b, "source": "secondary_rpc"},
        ]

        route, _ = threshold_routing(
            payload={"wallet": wallet_address, "chain": chain},
            rpc_responses=rpc_responses,
        )

        if route == "reject":
            logger.error("[EtherscanAPITool] Telemetry REJECTED by anti-spoofing layer")
            return ToolOutput(success=False, data={}, confidence_score=0.0,
                              error="[AntiSpoof] Payload rejected: critical integrity failure")

        if route == "fallback":
            logger.warning("[EtherscanAPITool] RPC consensus FAILED — routing to fallback (empty result)")
            return ToolOutput(success=False, data={"fallback": True}, confidence_score=0.0,
                              error="[AntiSpoof] RPC consensus failed: data may be spoofed/jammed")

        # Consensus OK (“proceed” route) — fetch full transaction list
        all_txs: list[dict] = []
        page = 1
        ...
        # Build hop list with live USD conversion
        hops = []
        for tx in all_txs[:max_hops]:
            ...
            hops.append({
                ...
                "value_usd":    convert_native_to_usd(value_native, coin),
        ...
        # Sign outgoing payload (tamper-evidence for downstream consumers)
        payload_out["_signature"] = sign_payload({k: v for k, v in payload_out.items()
                                                    if k != "_signature"})
    ...
    def _fetch_evm_txs(self, wallet_address: str, base_url: str, api_key: str,
                       page: int = 1, limit: int = 10) -> list[dict]:
        """Fetch a small batch of transactions from an EVM RPC endpoint."""
        try:
