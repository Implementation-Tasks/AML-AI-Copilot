...
# ─── CIWS Background SAR Report Writer ───────────────────────────────────────

# Module-level executor for fire-and-forget async crew runs
# (daemon=True so it doesn't block Quapp process shutdown)
_background_executor = ThreadPoolExecutor(max_workers=4, thread_name_prefix="CIWS-SAR")


def _fire_async_sar_report(agent_input, trace_id: str) -> None:
    """
    Launch CrewAI analysis in a non-blocking background thread.

    This is the ANALYSIS CHAIN — runs post-engagement after FREEZE is already
    triggered. Generates the SAR-ready compliance report without blocking
    the FREEZE decision latency.

    Per CIWS Decoupling architecture (Bottleneck 2, slide 15):
        Kill Chain  : QUBO riskscore ≥ 0.85 → FREEZE (zero LLM latency)
        Analysis Chain: CrewAI async → SAR report (seconds to minutes, non-blocking)
    """
    def _run():
        try:
            logger.info(
                f"[CIWS-SAR] Async CrewAI started | trace_id={trace_id} "
                f"| wallet={agent_input.wallet_address}"
            )
            report = run_aml_crew(agent_input)
            logger.info(
                f"[CIWS-SAR] SAR report generated | case_id={report.case_id} "
                f"| risk={report.risk_level} | trace_id={trace_id}"
            )
        except Exception as exc:
            logger.error(
                f"[CIWS-SAR] Async SAR generation failed | trace_id={trace_id} | error={exc}",
                exc_info=True,
            )

    _background_executor.submit(_run)
    logger.info(
        f"[CIWS-SAR] Background SAR task submitted | trace_id={trace_id}"
    )
...
def handler(event: dict[str, Any]) -> dict[str, Any]:
    ...
    # ── 3. CIWS Kill Chain — Immediate FREEZE (no LLM latency) ──────────────
    #
    # Architecture (Bottleneck 2, slide 15):
    #   Kill Chain  : QUBO riskscore ≥ 0.85 → FREEZE immediately via smart-contract
    #   Analysis Chain: CrewAI runs async post-engagement → generates SAR report
    #
    # The FREEZE decision is DECOUPLED from CrewAI to achieve near-millisecond
    # interception. CrewAI then enriches the case asynchronously.

    if qubo_risk_score >= QUBO_RISK_THRESHOLD:
        logger.info(
            f"[Handler] ⚡ CIWS KILL CHAIN triggered | "
            f"riskscore={qubo_risk_score:.3f} >= threshold={QUBO_RISK_THRESHOLD} "
            f"| FREEZE issued immediately — zero LLM latency"
        )

        # Build immediate FREEZE response (Kill Chain output)
        elapsed_freeze = time.perf_counter() - pipeline_start
        freeze_result = {
            "status":              "freeze_triggered",
            "case_id":             f"AML-FREEZE-{trace_id[:8].upper()}",
            "wallet_address":      wallet_address,
            "risk_level":          "HIGH",
            ...
            "audit_hash":          None,
            "trace_id":            trace_id,
            "runtime_seconds":     round(elapsed_freeze, 3),
            "async_report_pending": True,   # SAR report being generated in background
            "error":               None,
        }

        # Fire-and-forget: launch CrewAI async for SAR report (Analysis Chain)
        try:
            ...
            logger.warning(
                f"[Handler] Failed to submit async SAR task: {exc} — FREEZE still issued"
            )

        logger.info(
            f"[Handler] FREEZE issued | case_id={freeze_result['case_id']} "
            f"| runtime={elapsed_freeze:.3f}s | SAR generation is async | trace_id={trace_id}"
        )
        return freeze_result

    # ── 4. Analysis Chain — Full CrewAI pipeline (for MONITOR / below threshold) ──
    #
    # For wallets that don't breach the FREEZE threshold, run the full
    # synchronous CrewAI pipeline to determine MONITOR vs CLEAR.
    report = None
    if mode == "hybrid":
        logger.info(
            f"[Handler] Activating CrewAI (sync) for sub-threshold analysis | "
            f"qubo_risk_score={qubo_risk_score:.3f} | mode={mode}"
        )
        try:
            agent_input = AgentInput(
                wallet_address=wallet_address,
        ...
        # Low risk / classical mode — QUBO alone is sufficient
        logger.info(
            f"[Handler] Risk score {qubo_risk_score:.3f} below threshold "
            f"{QUBO_RISK_THRESHOLD} in classical mode — skipping agent stage."
        )
